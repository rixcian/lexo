...
export type InputProps = Omit<
  ...
  "size"
> & {
  size?: "sm" | "default" | "lg" | "duo" | "duo-sm" | number;
  unstyled?: boolean;
  nativeInput?: boolean;
};
...
export function Input({
  ...
  const inputClassName = cn(
    ...
    // DESIGN.md section 4: inputs are chunky - 12x16 padding, radius 12. The
    // height has to live on the inner control, not the wrapper span, or the
    // text sits above the centre. 16px text at every breakpoint also stops iOS
    // zooming in when a field is focused.
    size === "duo" &&
      "h-12 px-4 text-base leading-12 sm:h-12 sm:text-base sm:leading-12",
    size === "duo-sm" &&
      "h-10 px-3.5 text-base leading-10 sm:h-10 sm:text-base sm:leading-10",
    props.type === "search" &&
